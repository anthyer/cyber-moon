...
- **Normalização de material:** os modelos chegam do glTF com `metallicFactor = 1.0`, o que faz o Godot tratar a cor do albedo como cor de reflexo em vez de cor difusa (fica escuro sem ambiente, ou espelha a cor do céu quando há um). Todo `.glb.import` de pacote Kenney aponta `import_script/path` para `res://scripts/utils/post_import_kenney.gd`, que zera o metálico, uniformiza rugosidade e filtro de textura, e também gera a colisão estática do modelo. Para cada malha encontrada, o script cria um `StaticBody3D` com forma trimesh e marca a metadata `superficie` (usada pelo sistema de audio para escolher o som de passo). Modelos de decoração atravessável (grama, flor, cogumelo e similares) ficam de fora por uma lista de trechos de nome no próprio script. Um pacote novo extraído para `assets/models/` precisa do mesmo `import_script/path` nos seus `.glb.import`:
  ```
